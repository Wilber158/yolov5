...
def gabor_kernel(frequency, sigma_x, sigma_y, theta=0, offset=0, ks=61, device=None):
    w = ks // 2
    grid_val = torch.arange(-w, w + 1, dtype=torch.float, device=device)
    x, y = torch.meshgrid(grid_val, grid_val, indexing='ij')  # Use 'indexing' for compatibility
    x = x.to(device)
    y = y.to(device)
